...
function reflowAndComposite(analysis, opts) {
  ...
  // Pre-pass: merge runs of narrow CENTERED text blocks into structured blocks
  // Author grids get fragmented by groupTextBlocks — merge consecutive centered
  // narrow blocks that together span a significant portion of the page width.
  // Only centered blocks qualify — this prevents body text columns from merging.
  const mergedMap = [];

  // Helper: check if a block is roughly centered on the page
  function isCentered(bbox) {
    const leftMargin = bbox.x;
    const rightMargin = pageWidth - (bbox.x + bbox.w);
    return leftMargin > pageWidth * 0.05 && Math.abs(leftMargin - rightMargin) < pageWidth * 0.15;
  }

  let mi = 0;
  while (mi < regionMap.length) {
    const region = regionMap[mi];
    // Only consider narrow, centered text blocks as candidates
    if (region.type !== "text" || region.block.bbox.w > pageWidth * 0.5 || !isCentered(region.bbox)) {
      mergedMap.push(region);
      mi++;
      continue;
    }

    // Collect consecutive narrow centered text blocks
    const run = [region];
    const startY = region.bbox.y;
    let mj = mi + 1;
    while (mj < regionMap.length && run.length < 20) {
      const next = regionMap[mj];
      if (next.type !== "text") break;
      // Must be centered (not a column-aligned body block)
      if (!isCentered(next.bbox) && next.block.bbox.w < pageWidth * 0.4) break;
      // Large vertical gap = section break
      const gap = next.gapAbsolute || 0;
      const avgFS = next.block.avgFontSize || 12;
      if (gap > avgFS * 2) break;
      // Stop if font size changes significantly
      ...
      // Stop if bold section heading appears after non-bold content
      if (next.block.isBold && !run[run.length - 1].block.isBold && run.length >= 3) break;
      // Wide blocks need multiple X clusters to be part of the grid
    ...
    if (run.length >= 3) {
      ...
      // Only merge if width spans enough and stays in top portion of page
      if (combinedW > pageWidth * 0.4 && combinedBottom < pageHeight * 0.6) {
